...
SCRIPT="${SCRIPT:-${BASE_DIR}/hpc_precompute_nn_pca_cpu.R}"
K="${K:-100}"
THREADS="${THREADS:-12}"
SEED="${SEED:-4}"
FORCE="${FORCE:-TRUE}"
...
NN_BACKEND="${NN_BACKEND:-faiss_hnsw,faiss_nndescent,faiss_ivf,faiss_flat_l2,faiss}"
LOG_DIR="${LOG_DIR:-${BASE_DIR}/precompute_logs_cpu12}"
...
if [[ ! -f "${SCRIPT}" ]]; then
  ...
  echo "Copy tools/hpc_precompute_nn_pca_cpu.R to ${SCRIPT} before running." >&2
  exit 1
fi

run_id="$(date +%Y%m%d_%H%M%S)"
log="${LOG_DIR}/precompute_nn_pca_cpu12_k${K}_${run_id}.log"
config="${LOG_DIR}/precompute_nn_pca_cpu12_k${K}_${run_id}.config"
...
export SINGULARITYENV_VECLIB_MAXIMUM_THREADS="${THREADS}"
export SINGULARITYENV_CUDA_VISIBLE_DEVICES=""

echo "Running CPU-only precompute job with ${THREADS} threads. Log: ${log}"
